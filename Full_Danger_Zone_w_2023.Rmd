...
date: '2022-11-23, adapted to include 2023 data 3/23/23'
output:
  html_document:
    toc: yes
    toc_depth: '4'
...
```{r Set-up Data}
inTrax <- read.csv("./TraxCalcRevised2023.csv")
...
datAll <- datAll %>%
  dplyr::select(!c(Track.length,Track.width,Time,X,Latitude,Longitude,Surface.temp,
...
GAMSegplot <- ggplot(data=Moo.both)+
  geom_point(aes(x=AvgDens020,y=AvgTrackDepth),color="#4a3e80",size=3,data=Moo)+
...
GAMSegplot <- ggplot(data=MooJ.both)+
  geom_point(aes(x=AvgDens020,y=AvgTrackDepth),color="#7a6cb8",size=3,data=MooJuv)+
...
Wlf.models <- as.data.frame(cbind(wlfR,exp(Wlf.gam.pred$fit)/65.7,
                                  exp(c(Wlf.gam.pred$fit-1.96*Wlf.gam.pred$se.fit))/65.7,
                                  exp(c(Wlf.gam.pred$fit+1.96*Wlf.gam.pred$se.fit))/65.7))
names(Wlf.models) <- c("AvgDens020","wlfpreds","wlfymin","wlfymax")

Moo.models <- as.data.frame(cbind(mooR,exp(Moo.gam.pred$fit)/123.8,
                                  exp(c(Moo.gam.pred$fit-1.96*Moo.gam.pred$se.fit))/123.8,
                                  exp(c(Moo.gam.pred$fit+1.96*Moo.gam.pred$se.fit))/123.8))
names(Moo.models) <- c("AvgDens020","moopreds","mooymin","mooymax")

MooJ.models <- as.data.frame(cbind(mooJR,exp(MooJ.gam.pred$fit)/123.8,
                                  exp(c(MooJ.gam.pred$fit-1.96*MooJ.gam.pred$se.fit))/123.8,
                                  exp(c(MooJ.gam.pred$fit+1.96*MooJ.gam.pred$se.fit))/123.8))
names(MooJ.models) <- c("AvgDens020","moojpreds","mooymin","mooymax")



Dee.models <- as.data.frame(cbind(deeR,exp(Dee.gam.pred$fit)/79.55,
                                  exp(c(Dee.gam.pred$fit-1.96*Dee.gam.pred$se.fit))/79.55,
                                  exp(c(Dee.gam.pred$fit+1.96*Dee.gam.pred$se.fit))/79.55))
names(Dee.models) <- c("AvgDens020","deepreds","deeymin","deeymax")


Bou.models <- as.data.frame(cbind(bouR,exp(Bou.gam.pred$fit)/101.2,
                                  exp(c(Bou.gam.pred$fit-1.96*Bou.gam.pred$se.fit))/101.2,
                                  exp(c(Bou.gam.pred$fit+1.96*Bou.gam.pred$se.fit))/101.2))
names(Bou.models) <- c("AvgDens020","boupreds","bouymin","bouymax")


Coy.models <- as.data.frame(cbind(coyR,exp(Coy.gam.pred$fit)/44,
                                  exp(c(Coy.gam.pred$fit-1.96*Coy.gam.pred$se.fit))/44,
                                  exp(c(Coy.gam.pred$fit+1.96*Coy.gam.pred$se.fit))/44))
names(Coy.models) <- c("AvgDens020","coypreds","coyymin","coyymax")

Bob.models <- as.data.frame(cbind(bobR,exp(Bob.gam.pred$fit)/43.7,
                                  exp(c(Bob.gam.pred$fit-1.96*Bob.gam.pred$se.fit))/43.7,
                                  exp(c(Bob.gam.pred$fit+1.96*Bob.gam.pred$se.fit))/43.7))
names(Bob.models) <- c("AvgDens020","bobpreds","bobymin","bobymax")


Cou.models <- as.data.frame(cbind(couR,exp(Cou.gam.pred$fit)/71.1,
                                  exp(c(Cou.gam.pred$fit-1.96*Cou.gam.pred$se.fit))/71.1,
                                  exp(c(Cou.gam.pred$fit+1.96*Cou.gam.pred$se.fit))/71.1))
...
deeDanger <- ggplot(data=all.models)+
  ...
  coord_cartesian(xlim=c(50,500),ylim=c(-.20,.20)) +
...
bouDanger <- ggplot(data=all.models)+
  ...
  labs(y=expression(atop("Sink Depth Difference", paste("(As % of Hind Leg Length)"))),
       x=bquote('Average Snow Density, Top 20cm (kg /'~m^3*')'))+
  theme_bw()+
  scale_colour_manual(values = cols1[3:4],name="Predator")+
  scale_y_continuous(labels = scales::percent)+
